  ...
  module RailsConfiguration
    def self.determine_database_adapter(app_name, app_config, app_root_path,node, options = {})
      ...
      if options[:force] || app_config[:database][:adapter].blank?
        ...
        if options[:consult_gemfile] and File.exists?("#{app_root_path}/Gemfile")
          bundle_list = `cd #{app_root_path}; #{node[:rails][:bundle_path]}/bundle list`
    ...
    def self.bundle(app_name, app_config, app_root_path, node)
      if File.exists?("#{app_root_path}/Gemfile")
        ...
        Chef::Log.info(OpsWorks::ShellOut.shellout("sudo su - #{app_config[:user]} -c 'cd #{app_root_path} && #{node[:rails][:bundle_path]}/bundle install --path #{app_config[:home]}/.bundler/#{app_name} --without=#{app_config[:ignore_bundler_groups].join(' ')}' 2>&1"))
      end
    end
  end
end
